...
export default function SellerQuotationDetailPage() {
  ...
  const getStatusBadge = (status) => {
    switch (status) {
      ...
      case 'rejected':
        return (
          <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-bold bg-rose-50 text-rose-705 border border-rose-250 uppercase">
    ...
    <div className="space-y-6 max-w-4xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 bg-white p-5 border border-slate-200 rounded-xl shadow-xs">
        <div>
          ...
          <h2 className="text-2xl font-black text-slate-800 tracking-tight mt-1">Quotation Profile</h2>
        </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-5 shadow-xs">
        <div className="flex items-center space-x-3 mb-4 border-b border-slate-100 pb-3">
          ...
          <div>
            <h3 className="font-bold text-slate-800 text-sm">Buyer Account (Customer)</h3>
            <p className="text-[10px] text-slate-400 font-bold uppercase">Linked Recipient Details</p>
          </div>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-slate-200 bg-slate-50/50 text-[10px] font-bold text-slate-400 uppercase tracking-wider">
                ...
                <th className="py-3 px-5">Unit Price (snapshot)</th>
            ...
            <tbody className="divide-y divide-slate-100 text-sm">
              {quotation.items?.map((item) => (
                <tr key={item.id} className="hover:bg-slate-50/20">
                  ...
                  <td className="py-4 px-5 text-slate-550 font-semibold">
